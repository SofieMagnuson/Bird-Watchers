...
public class Player : MonoBehaviour
{
    ...
    public GameObject skull, skullNoPoint, hunterSkull, windZone, skullhunter, poop, chosenSkull, tutorialText, loseText, loseScreen, pileOfSkulls, winText, loseBack, allTutorialText, loseWinButton, winBlack;
    public GameObject[] skulls, chosens, pictures, poofs, hairs, texts, accessories;
    ...
    void Update()
    {
        ...
        else if (!tutorialMode && !camScript.introMode && !showingHuman && !showedHuman)
        {
    ...
    private void CheckIfShowingHunter()
    {
    ...
    private void CheckAchievementList()
    {
    ...
    private void HandleTutorialHuman()
    {
        ...
        {
            if (skullNoPoint != null && allTutorialText != null)
            {
        ...
        if (inDropZone && allTutorialText != null && !tutorialMode && !showingHuman && reachedSkull)
        {
            if (texts[4] != null)
            {
    ...
    private void HandleTargetToSet()
    {
    ...
    private void RotateCamToAttackMode()
    {
    ...
    private void HandleTargetToKill()
    {
        if (targ == targets[0])
        {
    ...
    private void CheckIfSkullIsDroppedInNest()
    {
        if (setBoolToFalse <= 0)
        {
            droppedSkull = false;
    ...
    public void Attack()
    {
        ...
        {
            transform.position = Vector3.MoveTowards(startPos, endPos, attackSpeed * Time.deltaTime);
        }
        if (startPos == endPos)
        {
            targetIsSet = false;
